...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%archdec%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    lnsafterarchopen_w_xtradat = [");"
        ...
        "out_bsn <= out_sosi_arr(k).bsn;"
        "out_valid <= out_sosi_arr(k).valid ;"
        "out_sop <= out_sosi_arr(k).sop;"
        "out_eop <= out_sosi_arr(k).eop;"
        "out_empty <= out_sosi_arr(k).empty;"
        "out_channel <= out_sosi_arr(k).channel;"
        "out_err <= out_sosi_arr(k).err;"
        "end generate;"
		];
...
function chararr = mknprts(wbfctr,use_sep)
if use_sep
...
function achararr = mkarch(wbfctr,use_sep)
    if use_sep
