...
function delay_complex_config(this_block)
  ...
  filepath = fileparts(which('delay_complex_config'));
  this_block.setTopLevelLanguage('VHDL');
  delay_complex_blk = this_block.blockName;
  delay_complex_blk_parent = get_param(delay_complex_blk,'Parent');

  is_async = checkbox2bool(get_param(delay_complex_blk_parent,'is_async'));
  if(is_async)
    this_block.setEntityName('delay_complex_async');
  else
    this_block.setEntityName('delay_complex');
  end
  delay = get_param(delay_complex_blk_parent,'delay');
  bram_primitive = get_param(delay_complex_blk_parent,'bram_primitive');
  bram_latency = get_param(delay_complex_blk_parent,'bram_latency');
  ...
  this_block.addFileToLibrary([filepath '/../../casper_delay/delay_complex.vhd'], 'xil_defaultlib');
  ...
  this_block.addFileToLibrary([filepath '/../../misc/concat.vhd'], 'casper_misc_lib');
  this_block.addFileToLibrary([filepath '/../../misc/c_to_ri.vhd'], 'casper_misc_lib');
  this_block.addFileToLibrary([filepath '/../../misc/ri_to_c.vhd'], 'casper_misc_lib');
