function vhdlfile = bus_expand_arbitrary_code_gen(division_bit_widths)
    %gather all the string arrays required to write full file:
    filepathscript = fileparts(which('bus_expand_arbitrary_code_gen'));                 %get the filepath of this script (and thereby all scripts needed)
    ...
    vhdlfile = fullfile(vhdlfilefolder, [bdroot '_bus_expand_arbitrary.vhd']);           %filename for vhd file

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%upperdec%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
    upperlines = [
    "-- A VHDL implementation of the CASPER bus_expand_arbitrary block."
    "-- This file is generated by `bus_expand_arbitrary_code_gen.m`"
...
function chararr = mknprts(bit_widths)
    ...
    outdatchar = "\to_data_%d   : out std_logic_vector;";
...
function chararr = mknaliases(bit_widths)
    chararr = strings(length(bit_widths),0);
    outdatchar = "\talias a_data_%d : STD_LOGIC_VECTOR (%d-1 downto 0) is o_data_%d;";
    for i = 1:length(bit_widths)
...
function chararr = mknarch(bit_widths)
    ...
    outdatchar = "\ta_data_%d <= a_data(%d-1 downto %d);";
    for i = 1:length(bit_widths)
        chararr(i,1)=sprintf(outdatchar, i, msb_pos, msb_pos-bit_widths(i));
