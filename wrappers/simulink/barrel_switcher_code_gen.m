function [vhdlfile, entityname] = barrel_switcher_code_gen(unique_prefix, inout_port_count, inout_port_bit_width)
    %gather all the string arrays required to write full file:
    filepathscript = fileparts(which('barrel_switcher_code_gen'));                 %get the filepath of this script (and thereby all scripts needed)
    ...
    entityname = [unique_prefix '_barrel_switcher'];

    % vhdlfile = fullfile(vhdlfilefolder, [bdroot '_' unique_prefix '_barrel_switcher.vhd']);           %filename for vhd file
    vhdlfile = fullfile(vhdlfilefolder, [bdroot '_' entityname '.vhd']);           %filename for vhd file

    upperlines = [
    "-- A wrapper for the barrel_switcher block."
    "-- This file is generated by `barrel_switcher_wrappercode_gen.m`"
    ...
    % sprintf("  i_data_1 <= TO_SVEC(1, %d);", inout_port_bit_width)
    % sprintf("  i_data_2 <= TO_SVEC(2, %d);", inout_port_bit_width)
    % sprintf("  i_data_3 <= TO_SVEC(3, %d);", inout_port_bit_width)
    % sprintf("  i_data_4 <= TO_SVEC(4, %d);", inout_port_bit_width)
...
function chararr = mknprts(port_count, port_bit_width)
    ...
    for i = 1:port_count
        chararr(2*i-1,1)=sprintf(inport_str, i, port_bit_width);
        if (i ~= port_count)
            chararr(2*i,1)=sprintf(outport_str, i, port_bit_width);
        else
            chararr(2*i,1)=sprintf(strip(outport_str,';'), i, port_bit_width);
        end
    end
end
...
function chararr = mknassignments(port_count, port_bit_width)
    ...
    for i = 1:port_count
        ...
        chararr(port_count + i,1)=sprintf(outport_str, i, i-1);
